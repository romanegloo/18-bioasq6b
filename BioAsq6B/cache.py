...
class Cache(object):
    def __init__(self, args=None):
        self.args = args
        self.docs_file = os.path.join(PATHS['data_dir'], 'cache-docs.pkl')
        self.scores_file = os.path.join(PATHS['data_dir'], 'cache-scores.pkl')
        self.documents = dict()
        self.scores = dict()
        self.score_keys = ['retrieval']  # qasim/journal may will be appended
    ...
    def confirm_updates(self):
        ...
        for reranker in self.args.rerank:
            if reranker == 'qasim':
                self.score_keys.append('qasim')
                self.flg_update_scores['qasim'] = False
            if reranker == 'journal':
                self.score_keys.append('journal')
                self.flg_update_scores['journal'] = False
    ...
    def update_scores(self, res):
        ...
        if 'qasim' in res.update_cache:
            self.scores[qid]['scores-qasim'] = res.scores['qasim']
            self.scores_changed = True
